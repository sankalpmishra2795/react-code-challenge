...
function Planets(props) {
    ...
    actions: [
      {
        ...
        action: (row) => { 
          if(row.films) {
            localStorage.setItem('films', JSON.stringify(row.films))
      ...
      },
      {
        ...
        action: (row) => {
           if(row.residents ) {
            localStorage.setItem('residents', JSON.stringify(row.residents))
  ...
  return (
    <div className='App'>
      <Grid data={{header:data.header, values: apiData,  actions: data.actions}} />
    </div>
  );
}
